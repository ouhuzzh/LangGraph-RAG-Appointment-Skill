...
class OfficialImportEntrypointTests(unittest.TestCase):
    def test_import_official_source_calls_importer_and_indexing(self):
        ...
        fake_result = mock.Mock(
            ...
            skipped=1,
            failed=0,
            discovered_url="manifest",
            written_files=[Path("alpha.md"), Path("beta.md")],
        )
        with mock.patch("core.document_manager.MedlinePlusXmlImporter") as importer_cls:
            ...
            with mock.patch.object(manager, "_index_markdown_paths", return_value={"processed": 2, "added": 2, "skipped": 0}) as index_mock:
        ...
        index_mock.assert_called_once_with(fake_result.written_files, skip_existing=False)
